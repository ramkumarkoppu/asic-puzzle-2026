...
> **Note: the solution code described in this writeup was published to this repository
> on September 6, 2026 — after the puzzle's September 4 submission deadline, per its
> no-spoilers rule.** Before that date only this writeup, its figures and `SOLUTION.md`
> were public. The writeup is self-contained: everything needed to follow the solution,
> including the answer, is on this page, and every source link below now resolves.
...
unmodified (its `README.md` gains only a pointer to this solution); everything under
`tools/out/` is regenerated by the commands below, and
